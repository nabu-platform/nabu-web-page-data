...
<template id="data-common-content">
	<div>
		...
		<div class="data-common-footer">
			<ul class="is-menu" v-if="data.globalActions.length" :class="getChildComponentClasses('data-button-container')">
				<li class="is-column" v-for="action in data.globalActions"><component
					:is="action.type == 'link' ? 'a' : 'button'"
					:disabled="action.useSelection && !data.selected.length"
					:class="[action.class, {'has-icon': action.icon}, getChildComponentClasses('data-table-button-' + data.cell.state.actions.indexOf(action))]"
					class="is-button"
					href="javascript:void(0)"
					v-action="function() { data.trigger(action) }"><icon v-if="action.icon" :name="action.icon"/><span class="is-text" v-if="action.label">{{$services.page.translate(action.label)}}</span></component></li>
			</ul>
		</div>
	</div>
</template>
